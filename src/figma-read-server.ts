...
function notConnected(): ReturnType<typeof errorResult> {
...
server.setRequestHandler(CallToolRequestSchema, async ({ params: { name, arguments: args } }) => {
  if (name === 'figma_status') {
    ...
    return { content: [{ type: 'text', text: JSON.stringify({ bridgePort: bridge.port || CONFIG.PORT, pluginConnected: connected, pluginInfo, readOnly: true, hint: connected ? 'CONNECTED. Read operations only.' : 'Run the Figma UI MCP Bridge plugin in Figma Desktop.' }, null, 2) }] }
  }
  ...
  if (name === 'figma_rules') {
    if (!(await bridge.isPluginConnected())) return notConnected()
    const sessionId = (args as Record<string, any> | undefined)?.sessionId
    try {
