	...
	factory: (require) => {
		...
		*/
		...
		function FigmaMcpPage() {
			...
			const [proxy, setProxy] = react.default.useState("");
			const [busy, setBusy] = react.default.useState("");
			...
			const apply = () => {
				...
				rpc(clientCtx.get("connection"), SIGHT_RPC.figmaMcpApply, req).then((value) => {
					setToken("");
					setMessage(value.ok ? `已写入 ${value.patchPath}。请重启 DSH Desktop 使 Figma MCP 工具生效。` : `写入失败: ${value.error ?? "unknown"}`);
					load();
				}).catch((e) => setError(e instanceof Error ? e.message : String(e))).finally(() => setBusy(""));
			};
			const remove = () => {
				...
				rpc(clientCtx.get("connection"), SIGHT_RPC.figmaMcpRemove, {}).then((value) => {
					setMessage(value.ok ? `已从 ${value.patchPath} 移除 Figma MCP 配置。` : `移除失败: ${value.error ?? "unknown"}`);
					load();
				}).catch((e) => setError(e instanceof Error ? e.message : String(e))).finally(() => setBusy(""));
			};
			const children = [];
			...
			children.push(react.default.createElement("p", { style: {
				...
				opacity: .75,
				lineHeight: 1.6
			} }, "把 Figma 设计稿数据接入模型：填写 Figma Personal Access Token 后写入 profile 的 cordis.patch.yml，通过 DSH 内置 mcp-client 挂载 Figma MCP 工具（get_figma_data / download_figma_images）。需要网络代理时填写代理地址。写入后请重启 DSH Desktop 生效。"));
			children.push(react.default.createElement("div", { style: {
				...GROUP,
				marginTop: 4
			} }, react.default.createElement("div", { style: GROUP_HEAD }, react.default.createElement("span", null, "连接"), status === null ? null : status.configured ? react.default.createElement(Chip, { tone: "on" }, "已配置") : react.default.createElement(Chip, { tone: "off" }, "未配置")), react.default.createElement("div", { style: {
				padding: "10px 12px",
				display: "flex",
				flexDirection: "column",
				gap: 8
			} }, status !== null && status.error !== null ? react.default.createElement("div", { style: {
				color: "#ef4444",
				fontSize: 12
			} }, `读取配置失败: ${status.error}`) : null, react.default.createElement("input", {
				type: "password",
				...
				style: INPUT_STYLE
			}), status !== null && status.configured ? react.default.createElement("div", { style: {
				fontSize: 12,
				opacity: .75,
				display: "flex",
				flexDirection: "column",
				gap: 2
			} }, react.default.createElement("span", null, `配置文件: ${status.patchPath}`), react.default.createElement("span", null, `Token: ${status.hasToken ? "已配置" : "缺失"} · 代理: ${status.proxy ?? "无"}`)) : null, message !== null ? react.default.createElement("div", { style: {
				fontSize: 12,
				color: "#4ade80",
				background: "rgba(34,197,94,0.08)",
				border: "1px solid rgba(34,197,94,0.35)",
				borderRadius: 6,
				padding: "6px 10px"
			} }, message) : null, error !== null ? react.default.createElement("div", { style: {
				fontSize: 12,
				color: "#ef4444"
				...
				disabled: busy !== "",
				onClick: apply
			}, busy === "apply" ? "写入中…" : "写入配置"), status !== null && status.configured ? react.default.createElement("button", {
				type: "button",
				style: BUTTON,
				disabled: busy !== "",
				onClick: remove
			}, busy === "remove" ? "移除中…" : "移除配置") : null))));
			return react.default.createElement("div", { style: {
				display: "flex",
				flexDirection: "column",
				gap: 14,
