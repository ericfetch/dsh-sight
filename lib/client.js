	...
	factory: (require) => {
		...
		*/
		...
		function SightPage() {
			...
			const [figmaStatus, setFigmaStatus] = react.default.useState(null);
			const [figmaToken, setFigmaToken] = react.default.useState("");
			const [figmaProxy, setFigmaProxy] = react.default.useState("");
			const [figmaMessage, setFigmaMessage] = react.default.useState(null);
			const [figmaError, setFigmaError] = react.default.useState(null);
			...
			const applyFigma = () => {
				if (busy !== "") return;
				if (figmaToken.trim().length === 0) {
					setFigmaError("请填写 Figma Personal Access Token");
					return;
				}
				setBusy("figma-apply");
				...
				const req = trimmedProxy.length > 0 ? {
					token: trimmedToken,
					proxy: trimmedProxy
				} : { token: trimmedToken };
				rpc(clientCtx.get("connection"), SIGHT_RPC.figmaMcpApply, req).then((value) => {
					setFigmaToken("");
					setFigmaMessage(value.ok ? `已写入 ${value.patchPath}。请重启 DSH Desktop 使 Figma MCP 工具生效。` : `写入失败: ${value.error ?? "unknown"}`);
					loadFigma();
				}).catch((e) => setFigmaError(e instanceof Error ? e.message : String(e))).finally(() => setBusy(""));
			};
			const removeFigma = () => {
				if (busy !== "") return;
				...
				setFigmaMessage(null);
				setFigmaError(null);
				rpc(clientCtx.get("connection"), SIGHT_RPC.figmaMcpRemove, {}).then((value) => {
					setFigmaMessage(value.ok ? `已从 ${value.patchPath} 移除 Figma MCP 配置。` : `移除失败: ${value.error ?? "unknown"}`);
					loadFigma();
				}).catch((e) => setFigmaError(e instanceof Error ? e.message : String(e))).finally(() => setBusy(""));
			};
			const children = [];
			...
			children.push(react.default.createElement("div", { style: {
				...GROUP,
				marginTop: 8
			} }, react.default.createElement("div", { style: GROUP_HEAD }, react.default.createElement("span", null, "Figma MCP"), figmaStatus === null ? null : figmaStatus.configured ? react.default.createElement(Chip, { tone: "on" }, "已配置") : react.default.createElement(Chip, { tone: "off" }, "未配置")), react.default.createElement("div", { style: {
				padding: "10px 12px",
				display: "flex",
				...
				opacity: .8,
				lineHeight: 1.6
			} }, "把 Figma 设计稿数据接入模型：填写 Figma Personal Access Token 后一键写入 profile 的 cordis.patch.yml，通过 DSH 内置 mcp-client 挂载 Figma MCP 工具 （get_figma_data / download_figma_images）。需要网络代理时填写代理地址。写入后请重启 DSH Desktop 生效。"), figmaStatus !== null && figmaStatus.error !== null ? react.default.createElement("div", { style: {
				color: "#ef4444",
				fontSize: 12
			} }, `读取配置失败: ${figmaStatus.error}`) : null, react.default.createElement("input", {
				type: "password",
				...
				value: figmaProxy,
				onChange: (e) => setFigmaProxy(e.target.value),
				style: INPUT_STYLE
			}), figmaStatus !== null && figmaStatus.configured ? react.default.createElement("div", { style: {
				fontSize: 12,
				opacity: .75,
				display: "flex",
				flexDirection: "column",
				gap: 2
			} }, react.default.createElement("span", null, `配置文件: ${figmaStatus.patchPath}`), react.default.createElement("span", null, `Token: ${figmaStatus.hasToken ? "已配置" : "缺失"} · 代理: ${figmaStatus.proxy ?? "无"}`)) : null, figmaMessage !== null ? react.default.createElement("div", { style: {
				fontSize: 12,
				color: "#4ade80",
				background: "rgba(34,197,94,0.08)",
				border: "1px solid rgba(34,197,94,0.35)",
				borderRadius: 6,
				padding: "6px 10px"
			} }, figmaMessage) : null, figmaError !== null ? react.default.createElement("div", { style: {
				fontSize: 12,
				color: "#ef4444"
			} }, figmaError) : null, react.default.createElement("div", { style: {
				display: "flex",
				gap: 8
			} }, react.default.createElement("button", {
				type: "button",
				...
				disabled: busy !== "",
				onClick: applyFigma
			}, busy === "figma-apply" ? "写入中…" : "写入配置"), figmaStatus !== null && figmaStatus.configured ? react.default.createElement("button", {
				type: "button",
				style: BUTTON,
				disabled: busy !== "",
				onClick: removeFigma
			}, busy === "figma-remove" ? "移除中…" : "移除配置") : null))));
