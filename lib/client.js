	...
	factory: (require) => {
		...
		*/
		...
		function FigmaMcpPage() {
			...
			const [busy, setBusy] = react.default.useState("");
			const [readToken, setReadToken] = react.default.useState("");
			const [readProxy, setReadProxy] = react.default.useState("");
			...
			const [error, setError] = react.default.useState(null);
			const load = react.default.useCallback(() => {
				rpc(clientCtx.get("connection"), SIGHT_RPC.figmaMcpStatus, {}).then((value) => setStatus(value)).catch((e) => setError(e instanceof Error ? e.message : String(e)));
			}, []);
			react.default.useEffect(() => {
				load();
			}, [load]);
			const applyRead = () => {
				if (busy !== "") return;
				if (readToken.trim().length === 0) {
					setError("请填写 Figma Personal Access Token");
					return;
				}
				setBusy("read");
				setReadMessage(null);
			...
			children.push(react.default.createElement("div", { style: GROUP }, react.default.createElement("div", { style: GROUP_HEAD }, react.default.createElement("span", null, "① 设计稿 → 代码"), react.default.createElement("span", { style: {
				fontSize: 11,
				opacity: .6
			} }, "只填 Token"), readCfg === void 0 ? null : readCfg.configured ? react.default.createElement(Chip, { tone: "on" }, "已启用") : react.default.createElement(Chip, { tone: "off" }, "未启用")), react.default.createElement("div", { style: {
				padding: "10px 12px",
				display: "flex",
				flexDirection: "column",
				gap: 8
				...
				opacity: .75,
				lineHeight: 1.6
			} }, "让模型读取 Figma 设计稿并生成代码。只需要一个 Figma Personal Access Token，无需安装任何插件。"), react.default.createElement("input", {
				type: "password",
				...
				value: readProxy,
				onChange: (e) => setReadProxy(e.target.value),
				style: inputStyle
				...
				disabled: busy !== "",
				onClick: applyRead
			}, busy === "read" ? "写入中…" : "启用"), readCfg !== void 0 && readCfg.configured ? react.default.createElement("button", {
				type: "button",
				style: BUTTON,
				disabled: busy !== "",
				onClick: removeRead
