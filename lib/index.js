		...
		*/
		const findFigmaRow = (patch) => {
			for (const entry of patch) {
				...
				for (let i = 0; i < list.length; i++) {
					...
					if (config.serverName === "figma-ui") return {
						insertEntry: e,
						row: r,
						index: i
					};
		...
		const figmaMcpStatus = () => {
			...
			try {
				...
				return {
					configured: findFigmaRow(patch) !== null,
					patchPath: file,
		...
		const handler = async (endpoint, payload) => {
			try {
				switch (endpoint) {
					...
					case SIGHT_RPC.figmaMcpApply: return ok(await figmaMcpApply());
					case SIGHT_RPC.figmaMcpRemove: return ok(figmaMcpRemove());
