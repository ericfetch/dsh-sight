...
function apply(ctx) {
	ctx.inject([
		...
		const deepseekDeclaresImage = (model) => Array.isArray(model?.inputModalities) && model.inputModalities.includes("image");
		...
		const writeModelVision = async (provider, model, vision) => {
			...
			if (profile === void 0 || typeof profile !== "object") {
				if (provider === DEEPSEEK_PROVIDER) {
					...
					if (!Array.isArray(models) || models.length === 0) throw new Error(`provider "${provider}" is not configured under ${DEEPSEEK_NS}`);
					if (models.find((m) => m !== null && typeof m === "object" && m.id === model) === void 0) throw new Error(`model "${model}" is not listed in ${DEEPSEEK_NS} models`);
					...
					await settings.mutate(DEEPSEEK_NS, [{
						op: "set",
						path: ["models"],
						value: next
					}]);
		...
		*/
		const applyDictionaryToDeepseek = async (seed) => {
			const models = deepseekSection()?.models;
			...
			return {
				applied: seed.applied + imageModels,
				providers: seed.providers + 1
			};
		};
		...
		const applyDictionary = async () => {
			...
			if (rawProviders === void 0 || typeof rawProviders !== "object") return applyDictionaryToDeepseek({
				applied,
			...
			return applyDictionaryToDeepseek({
				applied,
				providers: touchedProviders
			});
		};
		...
		const findFigmaRow = (patch, mode) => {
			...
			for (const entry of patch) {
				...
				for (let i = 0; i < list.length; i++) {
					...
					if (config.serverName === serverName) return {
						insertEntry: e,
						row: r,
						index: i
					};
